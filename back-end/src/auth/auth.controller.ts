...
export class AuthController {
  ...
  async register(@Body() body: RegisterDto) {
    console.log('🔵 [AUTH] POST /auth/register - Request:', {
      email: body.email,
  ...
  async googleCallback(@Req() req: any, @Res() res: Response) {
    ...
    const { access_token } = await this.authService.socialLogin(
      'google',
      req.user,
    );
    console.log(
      '🟢 [GOOGLE CALLBACK] Generated JWT token (length):',
      access_token.length,
    );
  ...
  facebookLogin(@Query('redirect') redirect: string, @Res() res: Response) {
    if (redirect) {
      ...
      console.log(
        '🔵 [FACEBOOK LOGIN] Stored redirect URI in cookie:',
        redirect,
      );
    }
  }
  ...
  async facebookCallback(@Req() req: any, @Res() res: Response) {
    ...
    console.log(
      '🔵 [FACEBOOK CALLBACK] User profile:',
      JSON.stringify(
        {
          id: req.user?.id,
          email: req.user?.emails?.[0]?.value,
